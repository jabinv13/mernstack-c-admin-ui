...
function findMorningEvening(): string {
  const now = new Date();
...
const timeOfDay = findMorningEvening();

console.log(timeOfDay); // "morning" or "afternoon" or "evening"
...
function HomePage() {
  const wish = findMorningEvening();
  const { user } = useAuthStore();
  return (
    <div>
      <Title level={4}>
        Welcome Good {wish} <br /> {user?.firstName}😀
      </Title>
